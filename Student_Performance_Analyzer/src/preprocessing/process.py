...
def average_marks(student_df):
    grade_ranges = [(60,100),(0,59)]
    grade_labels = ['Pass','Fail']

    # Create a new column 'math_grade' based on math scores
    def categorize_overall_score(score):
        for i, (lower, upper) in enumerate(grade_ranges):
            if lower <= score <= upper:
                return grade_labels[i]
        return 'Fail'
    student_df['overall_marks'] = (student_df['math score']+student_df['reading score'] + student_df['writing score'])/3
    student_df['overall_grade'] = student_df['overall_marks'].apply(categorize_overall_score)
